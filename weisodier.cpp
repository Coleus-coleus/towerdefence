...
//        return;
    ...
    QRect MaxHPRect(HPPoint,QSize(100,2));
    painter->drawRect(MaxHPRect);
    painter->setBrush(Qt::blue);
    QRect CurrentHPRect(HPPoint,QSize((double)WeiSodier_CurrentHP/WeiSodier_MaxHP*100,2));
...
//    painter->drawPixmap(OffsetPoint,WeiSodier_Picture);
    painter->drawPixmap(WeiSodier_Coordinate,WeiSodier_Picture);
    painter->restore();
}
...
//            WeiSodier_DestinationRoutePoint=WeiSodier_DestinationRoutePoint->NextRoutePoint();//目的地为下一个屯田
//    }
    QPoint AimRoutePoint=WeiSodier_DestinationRoutePoint->Coordinate();
    //向量标准化
    qreal MarchingSpeed=WeiSodier_Speed;
